...
if($OBJ['data_type'] == "checkout")
{
    ...
    $db = new Database(); // Connexion établie une seule fois
    ...
    foreach ($data as $row) {
        ...
        if(is_array($check))
        {
            ...
            // 1. Insertion de l'article dans la table des ventes (sales)
            ...
            // 2. Mise à jour du compteur de vues
            $query = "update products set views = views + 1 where id = :id limit 1";
            $db->query($query,['id'=>$check['id']]);

            // 3. Mise à jour du stock
            $query = "UPDATE products SET qty = qty - $qty WHERE id = :id LIMIT 1";
            $db->query($query,['id'=>$check['id']]);


            // LIGNES À SUPPRIMER OU COMMENTER : 
            // $db = new Database; 
