<?php require views_path('partials/header');?>
	...
	<div class="d-flex">
		<div style="min-height:600px;" class="shadow-sm col-7 p-4">
			...
			<div class="input-group mb-3"><h3> Produits </h3>
		...
		<div class="col-5 bg-light p-4 pt-2">
			...
			<div class="table-responsive" style="height:400px;overflow-y: scroll;">
				<table class="table table-striped table-hover">
					<tr>
						<th>Image</th><th>Description</th><th>Prix</th>
					</tr>
			...
			<div class="">
				<button onclick="show_modal('amount-paid')" class="btn btn-success my-2 w-100 py-4">Valider</button>
				<button onclick="clear_all()" class="btn btn-primary my-2 w-100">Effacer</button>
			</div>
		</div>
	</div>	
...
<!--modals-->
	...
	<div role="close-button" onclick="hide_modal(event,'amount-paid')" class="js-amount-paid-modal hide" style="animation: appear .5s ease;background-color: #000000bb; width: 100%;height: 100%;position: fixed;left:0px;top:0px;z-index: 4;">
		...
		<div style="width:500px;min-height:200px;background-color:white;padding:10px;margin:auto;margin-top:100px">
			...
			<br>
			<input onkeyup="if(event.keyCode == 13)validate_amount_paid(event)"  type="text" class="js-amount-paid-input form-control" placeholder="Enter amount paid" readonly>
			<br>
			<button role="close-button" onclick="hide_modal(event,'amount-paid')"  class="btn btn-secondary">Annuler</button>
			<button onclick="validate_amount_paid(event)" class="btn btn-primary float-end">Valider</button>
		</div>
	</div>
...
            amount_paid_input.value = GTOTAL.toFixed(2); // Affiche le total formaté
	...
	function validate_amount_paid(e)
	{
		...
		if(amount < GTOTAL){
			...
			alert("Le total n'est pas correct");
			return;
		}
