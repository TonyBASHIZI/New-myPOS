<?php require views_path('partials/header');?>
	...
	<div class="container-fluid border rounded p-4 m-2 col-lg-4 mx-auto">
		...
		<form method="post" enctype="multipart/form-data">
			...
			<div class="mb-3">
			  <label for="barcodeControlInput1" class="form-label">Lot <small class="text-muted">(optional)</small></label>
			  <input value="<?=set_value('lot',$row['lot'])?>" name="lot" type="text" class="form-control <?=!empty($errors['lot']) ? 'border-danger':''?>" id="barcodeControlInput1" value="<?=set_value('lot',$row['lot'])?>">
			  <?php if(!empty($errors['lot'])):?>
					<small class="text-danger"><?=$errors['lot']?></small>
				<?php endif;?>
			</div>
			<div class="mb-3">
			  <label for="barcodeControlInput1" class="form-label">Shop <small class="text-muted">(optional)</small></label>
			  <input value="<?=set_value('shop',$row['shop'])?>" name="shop" type="text" class="form-control <?=!empty($errors['shop']) ? 'border-danger':''?>" id="barcodeControlInput1" value="<?=set_value('shop',$row['shop'])?>">
			  <?php if(!empty($errors['shop'])):?>
					<small class="text-danger"><?=$errors['shop']?></small>
				<?php endif;?>
			</div>
			<div class="mb-3">
			  <label for="barcodeControlInput1" class="form-label">Barcode <small class="text-muted">(optional)</small></label>
			  <input value="<?=set_value('barcode',$row['barcode'])?>" name="barcode" type="text" class="form-control <?=!empty($errors['barcode']) ? 'border-danger':''?>" id="barcodeControlInput1" placeholder="Product barcode" readonly=readonly>
